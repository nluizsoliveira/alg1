...
from project.tests.integration_test import INPUTS_PATH, OUTPUT_FILES_PATH, DUMPED_STDOUT_PATH

inputs_filenames = listdir(INPUTS_PATH)
for index, input_filename in enumerate(inputs_filenames, start=1):
    orchestrator = Orchestrator(
        f'{OUTPUT_FILES_PATH}{index}/', 
        debug=True,
        debug_path =  f'{DUMPED_STDOUT_PATH}{index}'
    )
